...
def _pick_claims(claims, priorities: list[str] | None, n: int = 2):
    if priorities:
        matched = [c for c in claims if c.category in priorities]
        rest = [c for c in claims if c.category not in priorities]
...
def recommend(
    scored: list[tuple[ProductClaims, str]],
    ...
    for pc, tier in scored:
        ps = scoring.score_product(pc, priorities)
    ...
    for idx, (pc, tier, ps, conf) in enumerate(enriched):
        ...
        top_pros = _pick_claims(pc.pros, priorities, 3)
        top_cons = _pick_claims(pc.cons, priorities, 3)
        rationale = _rationale(pc.product, bucket, top_pros, top_cons)
        bd = scoring.breakdown(pc, priorities)
        recs.append(Recommendation(
            product=pc.product, tier=tier, bucket=bucket, confidence=conf,
            product_score=bd["net_score"], rationale=rationale,
